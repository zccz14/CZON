---
"title": "CZON - Présentation du moteur de contenu Markdown AI-Native"
"summary": "CZON est un moteur de contenu Markdown AI-Native conçu pour aider les créateurs de contenu à créer et gérer du contenu de manière plus intelligente à l'ère de l'IA. Il met l'accent sur un retour à l'essence du contenu, en minimisant les distractions pour que l'utilisateur se concentre sur l'écriture, tout en confiant des tâches comme la génération de titres, l'extraction de résumés, la classification par étiquettes et l'organisation de la navigation au LLM pour un traitement automatique. CZON masque les configurations complexes, simplifie la structure du projet, prend en charge l'écriture dans la langue maternelle et utilise l'IA pour la traduction multilingue. Il intègre une fonctionnalité de génération de site statique, mais se concentre sur l'amélioration de la qualité et de l'accessibilité du contenu plutôt que sur des personnalisations complexes. La documentation fournit également des étapes pour démarrer rapidement, incluant la configuration des variables d'environnement et les commandes de construction."
"tags":
  - "IA"
  - "Markdown"
  - "Moteur de contenu"
  - "Traduction multilingue"
  - "Génération de site statique"
  - "Création de contenu"
  - "LLM"
---

# CZON - Moteur de contenu Markdown AI-Native

[> Démo du site web](https://czon.zccz14.com/)

À l'ère de l'IA, en tant que créateur de contenu pour sites web, nous pouvons disposer d'un moteur de création de contenu plus intelligent.

**Retour au contenu : Minimiser les distractions, se concentrer sur l'écriture**

Rendre la documentation à son essence, rendre l'écriture à sa tranquillité.

Premièrement, je ne veux pas être un bibliothécaire. Parfois, je veux écrire du contenu, mais je n'ai peut-être pas encore réfléchi à son titre, ni à une organisation particulière. J'aimerais que le LLM génère automatiquement des titres, crée des résumés, effectue des classifications, organise la navigation et d'autres tâches de tri.

Deuxièmement, mon espace de travail doit être rangé. Je ne veux pas d'outils de construction à configuration complexe, je ne veux pas me battre avec des configurations de documentation compliquées, et je n'aime pas les structures complexes. Chaque fois que je vois les fichiers de configuration et les structures de projet d'outils comme Docusaurus, VuePress, Astro, j'ai mal à la tête. Maintenant, CZON choisit de cacher toute la configuration dans le répertoire `.czon`, de la balayer dans un coin, pour que l'utilisateur se concentre uniquement sur l'écriture du contenu, et laisse CZON gérer le reste. Tous les autres répertoires sont votre espace d'écriture, vous pouvez organiser vos fichiers Markdown comme bon vous semble.

**Retour à la langue maternelle : Écrire dans sa langue maternelle, construire des versions multilingues de manière fluide**

La vie est courte, je suis trop paresseux pour traduire. C'est dans sa langue maternelle qu'une personne peut exploiter au mieux la profondeur de sa réflexion. Mais en même temps, il est nécessaire de rester connecté au monde, et je ne veux pas renoncer à la communication multilingue. J'espère donc avoir un outil qui me permette d'écrire dans ma langue maternelle, tout en permettant à plus de personnes de voir mon contenu. Actuellement, l'i18n nécessite non seulement de traduire soi-même, mais aussi de maintenir les mises à jour du contenu, ce qui est trop fastidieux. Je choisis d'utiliser le LLM pour m'aider à traduire, ce qui me fait gagner beaucoup de temps et permet de réaliser des traductions multilingues en un clic.

## Fonctionnalités de base

1. 🌍 **Traduction multilingue automatique** : Utilise l'IA pour des traductions incrémentielles, permettant à l'utilisateur d'écrire en Markdown dans sa langue maternelle, tout en pouvant produire du contenu multilingue.
2. 💭 **Extraction automatique de résumés** : Utilise l'IA pour analyser et extraire le contenu du texte original.
3. 🏷️ **Classification automatique par étiquettes** : Utilise l'IA pour extraire et gérer les étiquettes et catégories du contenu.
4. 🧭 **Navigation par catégories intelligente** : Utilise l'IA pour générer un plan du site et une navigation, indépendamment de l'emplacement des fichiers sources.

## Génération de site statique (SSG)

CZON intègre une solution SSG qui peut transformer n'importe quel dossier git contenant des fichiers Markdown en un site HTML statique.

⚠️ Veuillez noter que CZON n'est pas un générateur de site statique (SSG) professionnel, mais un moteur de contenu Markdown piloté par l'IA, axé sur la création et la gestion de contenu.

✅ CZON vise à simplifier le processus de création et de publication de contenu, permettant aux utilisateurs de se concentrer sur l'écriture elle-même, sans être gênés par des configurations et des outils complexes.

❌ CZON ne fournira pas de personnalisations de thèmes complexes ni d'écosystème de plugins, mais se concentrera sur l'amélioration de la qualité et de l'accessibilité du contenu grâce à l'IA.

🔔 Cependant, CZON peut toujours générer un site statique. Si nécessaire, CZON peut être utilisé comme prétraitement et intégré à d'autres solutions SSG pour générer des sites personnalisés et esthétiques.

## Démarrage rapide

Prérequis :

- [Node.js](https://nodejs.org/) installé (version 18 ou supérieure recommandée, nécessite la commande npx)
- Clé API [OpenAI](https://platform.openai.com/account/api-keys) obtenue, ou une clé API compatible avec OPENAI
- Git installé (pour lister les fichiers Markdown à partir d'un dépôt Git, en ignorant les fichiers filtrés par les règles .gitignore)

Tout d'abord, travaillez dans un dossier déjà géré par git :

Configurez les variables d'environnement

```bash
export OPENAI_API_KEY="sk-xxxxxx"  # Obligatoire : remplacez par votre clé API OpenAI
export OPENAI_BASE_URL="https://api.openai.com/v1"  # Optionnel, utilise par défaut l'API officielle d'OpenAI
export OPENAI_MODEL="gpt-3.5-turbo"  # Optionnel, utilise par défaut le modèle gpt-3.5-turbo
```

Construisez le site, avec prise en charge de la traduction multilingue. Utilise le répertoire courant comme répertoire source, et sort le résultat dans le répertoire `.czon/dist`.

- Vous pouvez spécifier les versions linguistiques à générer en utilisant plusieurs fois le paramètre `--lang` (par exemple `--lang zh-Hans --lang en-US`).
- Aucune configuration de la langue source n'est nécessaire, CZON la détecte automatiquement.

```bash
npx czon@latest build --lang zh-Hans --lang en-US
```

**Voir plus de paramètres ou obtenir de l'aide** :

```bash
npx czon@latest
```