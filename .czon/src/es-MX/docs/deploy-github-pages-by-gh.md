...
Primero, instala la herramienta [GitHub CLI](https://cli.github.com/) (si aún no la tienes) y asegúrate de haber iniciado sesión (si aún no lo has hecho):

```bash
$ gh --version # Verifica la versión para asegurar que la instalación fue exitosa
```
...
Lo único que queda es ejecutar el siguiente comando y seguir las indicaciones:
