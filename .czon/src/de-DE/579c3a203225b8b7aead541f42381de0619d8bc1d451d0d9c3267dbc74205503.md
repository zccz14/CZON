---
'title': 'KaTeX-Formeldarstellungstest'
'summary': 'Dieses Dokument ist ein Beispiel zum Testen der KaTeX-Formeldarstellung. Es enthält mehrere Inline- und Block-Mathematikformeln wie E=mc^2, Integrale, Ableitungen, Reihen, stückweise definierte Funktionen, Tabellen, Matrizen usw., um die korrekte Darstellung der Formeln zu überprüfen.'
'tags':
  - 'KaTeX'
  - 'Formeltest'
  - 'Mathematische Formeln'
  - 'LaTeX'
  - 'Dokumentenanalyse'
  - 'Darstellungseffekt'
'inferred_lang': 'de-DE'
---

# KaTeX-Formeldarstellungstest

Dies ist ein Inline-Formeltest: $ E = mc^2 $.

Dies ist ein Block-Formeltest:

$$
\int_a^b f(x) \, dx = F(b) - F(a)
$$

Dies ist ein weiterer Block-Formeltest:

$$
\frac{d}{dx}e^x = e^x
$$

Dies ist ein komplexer Formeltest:

$$
\sum_{n=1}^{\infty} \frac{1}{n^2} = \frac{\pi^2}{6}
$$

Formeltest mit Fallunterscheidung:

$$
f(x) =
\begin{cases}
x^2 & \text{wenn } x \geq 0 \\
-x & \text{wenn } x < 0
\end{cases}
$$

Tabellenformel:

$$
\begin{array}{c|c|c}
a & b & c \\
\hline
1 & 2 & 3 \\
4 & 5 & 6 \\
\end{array}
$$

Matrixformel:

$$
\begin{pmatrix}
1 & 2 \\
3 & 4
\end{pmatrix}
$$

Formeltest mit Text:

$$
E = mc^2 \quad \text{wobei } m \text{ die Masse und } c \text{ die Lichtgeschwindigkeit ist}
$$

Verallgemeinerte Kelly-Formel:

$$
f^* = \argmax_f \sum_{i \in X} P_i \ln (1 + f \cdot R_i)
$$

Bitte stellen Sie sicher, dass alle oben genannten Formeln korrekt angezeigt werden.
