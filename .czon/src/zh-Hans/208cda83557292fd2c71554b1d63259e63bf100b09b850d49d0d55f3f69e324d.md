...
'summary':
  'CZON 是一个结合 AI 功能的静态站点生成器，专为 Markdown 文档设计。它允许用户将包含 Markdown 文件的 Git
  文件夹快速构建成静态 HTML 站点，无需复杂配置。核心功能包括智能分类导航，通过 AI
  分析内容自动生成站点地图和导航，以及自动增量翻译，支持用户用母语写作并一键生成多语言版本。工具强调回归内容本质，最小化写作干扰，并利用 LLM
  处理标题生成、摘要、翻译等任务，简化文档管理流程。快速开始部分提供了环境配置和构建命令的详细说明。'
'tags':
  - '静态站点生成器'
  - 'Markdown'
  - 'AI'
