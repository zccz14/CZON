...
"summary": "Este documento detalla los métodos para personalizar estilos en el proyecto CZON. Al crear un archivo style.css en el directorio .czon del proyecto y escribir código CSS, se pueden sobrescribir los estilos predeterminados o añadir apariencias personalizadas. El documento explica que CZON detecta y copia este archivo al directorio de salida durante la construcción, y añade un enlace de estilo en cada página HTML para garantizar que los estilos personalizados se carguen después de los estilos integrados, logrando así el efecto de sobrescritura. Se proporciona una lista de variables CSS disponibles, incluyendo colores de fondo, colores de texto, colores de enlaces, etc., y se muestran múltiples ejemplos prácticos, como personalizar colores de marca, ajustar el ancho del área de contenido, personalizar estilos de bloques de código y ocultar elementos específicos. Finalmente, se enfatizan las precauciones, como que el nombre del archivo debe ser style.css, que se requiere una reconstrucción después de las modificaciones, que se recomienda usar variables CSS para la sobrescritura y que puede ser necesario usar !important ya que CZON utiliza Tailwind CSS."
"tags":
  - "CZON"
...
CZON admite la personalización de estilos del sitio a través del archivo `.czon/style.css`, permitiéndote sobrescribir los estilos predeterminados o añadir una apariencia personalizada.
...
```css
/* Ejemplo: Modificar el color de los enlaces */
:root {
  --link-color: #0066cc;
}

/* Ejemplo: Modificar el fondo en modo oscuro */
html.dark {
  --bg-primary: #0d1117;
}
```
...
- En cada página HTML generada, se añade un enlace de estilo dentro de `<head>`:
  ```html
...
- Se recomienda personalizar los estilos sobrescribiendo variables CSS, ya que esto garantiza compatibilidad con los modos claro y oscuro.
