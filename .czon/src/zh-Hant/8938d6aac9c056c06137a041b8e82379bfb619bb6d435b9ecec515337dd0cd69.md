---
"title": "Mermaid圖表測試文件"
"summary": "本文檔是一個測試文件，專門用於驗證CZON平台中Mermaid圖表的渲染功能。文件展示了流程圖、序列圖、甘特圖、類圖、狀態圖和餅圖等多種圖表類型的範例，並包含一個錯誤語法測試以驗證錯誤處理機制。透過這些範例，文件旨在確保CZON平台能夠正確解析和顯示Mermaid圖表，從而支援使用者建立和分享複雜的可視化內容。文件還簡要說明了測試的目的和包含的圖表類型，幫助讀者理解其核心價值。"
"tags":
  - "Mermaid"
  - "圖表測試"
  - "CZON"
