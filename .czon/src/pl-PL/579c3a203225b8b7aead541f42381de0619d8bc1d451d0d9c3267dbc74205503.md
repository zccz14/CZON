---
'title': 'Test wyświetlania formuł KaTeX'
'summary':
  'Ten dokument to przykład testujący wyświetlanie formuł KaTeX, zawierający wiele formuł matematycznych w tekście i blokach, takich jak
  E=mc^2, całki, pochodne, szeregi, funkcje przedziałowe, tabele, macierze itp., mający na celu weryfikację poprawnego renderowania formuł.'
'tags':
  - 'KaTeX'
  - 'test formuł'
  - 'formuły matematyczne'
  - 'LaTeX'
  - 'analiza dokumentów'
  - 'efekt wyświetlania'
'inferred_lang': 'pl-PL'
---

# Test wyświetlania formuł KaTeX

To jest test formuły w tekście: $ E = mc^2 $.

To jest test formuły blokowej:

$$
\int_a^b f(x) \, dx = F(b) - F(a)
$$

To jest kolejny test formuły blokowej:

$$
\frac{d}{dx}e^x = e^x
$$

To jest test złożonej formuły:

$$
\sum_{n=1}^{\infty} \frac{1}{n^2} = \frac{\pi^2}{6}
$$

Test formuły z przypadkami:

$$
f(x) =
\begin{cases}
x^2 & \text{if } x \geq 0 \\
-x & \text{if } x < 0
\end{cases}
$$

Formuła tabeli:

$$
\begin{array}{c|c|c}
a & b & c \\
\hline
1 & 2 & 3 \\
4 & 5 & 6 \\
\end{array}
$$

Formuła macierzy:

$$
\begin{pmatrix}
1 & 2 \\
3 & 4
\end{pmatrix}
$$

Test formuły z tekstem:

$$
E = mc^2 \quad \text{where } m \text{ is mass and
    } c \text{ is the speed of light}
$$

Uogólniony wzór Kelly'ego:

$$
f^* = \argmax_f \sum_{i \in X} P_i \ln (1 + f \cdot R_i)
$$

Proszę upewnić się, że wszystkie powyższe formuły są poprawnie wyświetlane.
